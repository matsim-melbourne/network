addGtfsLinks <- function(outputLocation="./test/",
  ...
  # note SRL Business Case https://suburbanrailloop.vic.gov.au/Library/BusinessandInvestmentCase
  # gives the following parameters for Cheltenham to Box Hill operation (Main report, p166):
  # weekday operating hours 4.30am-12.30am, 6 minute service peak hours and 10 minute otherwise,
  # 22 minute trip Cheltenham to Box Hill
  ...
  HOURS <- c("04:30:00", "07:00:00", "09:00:00", "16:00:00", "18:00:00", "24:31:00")  # last departure 24:30
  INTERVAL <- c(600, 360, 600, 360, 600)  # 6 mins in peak periods; otherwise 10 mins

  SPEED <- 67.25  # km/h (this speed produces full journey time of 22 mins, matching SRL Business Case, see above)
...
exportGtfsSchedule <- function(links,
  ...
  departures <- ptNetworkDepartures %>%
    ...
    mutate(type=NA,
           ...
           type=ifelse(service_type=="SRL",1, type),
